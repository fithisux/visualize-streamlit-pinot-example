...
query = """
select count(*) FILTER(WHERE  timestamp > ago('PT1M')) AS events1Min,
        count(*) FILTER(WHERE  timestamp <= ago('PT1M') AND timestamp > ago('PT2M')) AS events1Min2Min,
        distinctcount(user) FILTER(WHERE  timestamp > ago('PT1M')) AS users1Min,
        distinctcount(user) FILTER(WHERE  timestamp <= ago('PT1M') AND timestamp > ago('PT2M')) AS users1Min2Min,
        distinctcount(domain) FILTER(WHERE  timestamp > ago('PT1M')) AS domains1Min,
        distinctcount(domain) FILTER(WHERE  timestamp <= ago('PT1M') AND timestamp > ago('PT2M')) AS domains1Min2Min
from wikievents_REALTIME 
where timestamp > ago('PT2M')
limit 1
"""
...
query = """
select ToDateTime(DATETRUNC('minute', timestamp*1000), 'yyyy-MM-dd hh:mm:ss') AS dateMin, count(*) AS changes, 
       distinctcount(user) AS users,
       distinctcount(domain) AS domains
from wikievents_REALTIME 
where timestamp > ago('PT10M')
group by dateMin
